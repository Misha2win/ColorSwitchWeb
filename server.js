import express, { json, static as exStatic } from 'express'
...
const levelsDirectory = join(staticDirectory, 'resources', 'levels')
const levelOrderPath = join(staticDirectory, 'resources', 'levelOrder.json')
...
app.get('/api/level-order', async (req, res) => {
    try {
        const contents = await readFile(levelOrderPath, 'utf8')
...
app.put('/api/level-order', async (req, res) => {
    ...
    try {
        await writeFile(levelOrderPath, `${JSON.stringify(levelOrderJSON, null, 3)}\n`, 'utf8')
...
app.put('/api/levels/:name', async (req, res) => {
    ...
    const filePath = join(levelsDirectory, `${name}.json`)

    try {
        await writeFile(filePath, `${JSON.stringify(levelJSON, null, 3)}\n`, 'utf8')
