...
document.getElementById('button-create-new').addEventListener('click', () => editor.handleCreateNewClick())
document.getElementById('current-level-label').addEventListener('change', event => editor.handleCurrentLevelSelectChange(event))
document.getElementById('button-previous-level').addEventListener('click', () => editor.handleLevelNavigation(-1))
document.getElementById('button-next-level').addEventListener('click', () => editor.handleLevelNavigation(1))
...
const levelUIButton = document.getElementById('button-level-ui-toggle')
levelUIButton.addEventListener('click', () => editor.toggleEditorLevelUI())
...
function setupEditorPlayControls() {
    ...
    const setButtonPressed = (button, isPressed) => {
        ...
        editor.playingLevel?.player?.setInputForKey(button.dataset.key, isPressed)
        editor.lastTime = performance.now()
    }
